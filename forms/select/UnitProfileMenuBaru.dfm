object frmProfileMenuBaru: TfrmProfileMenuBaru
  Left = 0
  Top = 0
  ClientHeight = 107
  ClientWidth = 426
  Caption = 'Menu Baru'
  BorderStyle = bsDialog
  OldCreateOrder = False
  MonitoredKeys.Keys = <>
  Font.Height = -13
  Font.Name = 'Roboto'
  OnBeforeShow = UniFormBeforeShow
  PixelsPerInch = 96
  TextHeight = 15
  object cbMenu: TUniDBLookupComboBox
    Left = 76
    Top = 18
    Width = 317
    Height = 27
    Hint = ''
    ListField = 'description'
    ListSource = dsmenu
    KeyField = 'id'
    ListFieldIndex = 0
    DataField = 'id'
    ParentFont = False
    Font.Height = -13
    Font.Name = 'Roboto'
    AnyMatch = True
    TabOrder = 0
    Color = clWindow
    Style = csDropDown
  end
  object UniLabel1: TUniLabel
    Left = 14
    Top = 30
    Width = 32
    Height = 15
    Hint = ''
    Caption = 'Menu'
    TabOrder = 1
  end
  object btnSimpan: TUniSpeedButton
    Left = 312
    Top = 73
    Width = 97
    Height = 26
    Hint = ''
    Glyph.Data = {
      
      D092EED093F2D396F7D79BF6D69BE6C48AEBB552FDF9F2FFFFFF}
    Caption = 'Simpan'
    ParentColor = False
    Color = clWindow
    TabOrder = 2
    OnClick = btnSimpanClick
  end
  object QMenu: TFDQuery
    Connection = UniMainModule.FDConnection1
    SQL.Strings = (
      
        'select menu_child.id,CONCAT(menu.description,'#39' > '#39',menu_child.de' +
        'scription) description'
      'from menu_child '
      'left join menu on menu_child.menu_id = menu.id'
      'order by menu.show_order,menu_child.sort_seq')
    Left = 128
    Top = 64
  end
  object dsmenu: TDataSource
    DataSet = QMenu
    Left = 184
    Top = 64
  end
  object cmd: TFDCommand
    Connection = UniMainModule.FDConnection1
    Left = 56
    Top = 64
  end
end
