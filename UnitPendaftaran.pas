...
uses
  ...
  uniDBComboBox, uniDBLookupComboBox, uniMemo, uniStringGrid;
...
begin
  ...
  QPaketDetail.ParamByName('panel_header_id').Value := QPaket.FieldByName('id')
    .AsInteger;
  QPaketDetail.ParamByName('urgency_id').Value := QCito.FieldByName('id')
    .AsInteger;
  QPaketDetail.ParamByName('tipe_pasien_id').Value :=
    QTipePasien.FieldByName('id').AsInteger;
  ...
  begin
    edDiscAmount.Text := FormatFloat('#,##0.00', QPaket.FieldByName('disc_amount').AsFloat);
